...
class ReviewsDB {
    ...
    editReview(review, callback) {db.query("update reviews set review = ?, rating = ?, date = ?, title = ? where id = ?", [review.getReview(), review.getRating(), review.getDate(), review.getTitle(), review.getId()], callback)}
    deleteReview(reviewId, callback) {
        db.query("delete from likes where reviewId = ?", [reviewId])
        db.query("delete from reviews where id = ?", [reviewId], callback)
    }
